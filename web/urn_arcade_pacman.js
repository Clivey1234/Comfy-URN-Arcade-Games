...
const EXTENSION_NAME = "urn.arcade.pacman";
const NODE_NAME = "URNArcadePacMan";
...
function createGame(node) {
    ...
    canvas.tabIndex = 0;
    canvas.setAttribute("aria-label", "URN Arcade PacMan game. Use arrow keys.");
    ...
    volumeSlider.setAttribute("aria-label", "URN Arcade PacMan volume");
...
// Ghosts.
    ...
    if (state === "idle" || state === "ready" || state === "paused" || state === "gameover") {
        ...
        let title = "READY!";
        if (state === "idle") title = "URN ARCADE PACMAN";
...
function attachGameSafely(node, attempt = 0) {
    if (!node || node.__urnArcadePacManGame || node.__urnArcadePacManAttachFailed) return;
    ...
    if (typeof node.addDOMWidget !== "function") {
        if (attempt < 40) {
            ...
            node.__urnArcadePacManAttachFailed = true;
            console.error("[URN Arcade PacMan] addDOMWidget was not available after 2 seconds.");
        }
        return;
    }
    ...
    try {
        ...
        node.__urnArcadePacManGame = game;
        ...
        console.info("[URN Arcade PacMan] game attached", node.id);
    } catch (err) {
        node.__urnArcadePacManAttachFailed = true;
        console.error("[URN Arcade PacMan] game UI attach failed; node kept alive:", err);
    }
}
...
app.registerExtension({
    ...
    async beforeRegisterNodeDef(nodeType, nodeData) {
        ...
        if (nodeType.prototype.__urnArcadePacManV8Patched) return;
        nodeType.prototype.__urnArcadePacManV8Patched = true;
        ...
        nodeType.prototype.onRemoved = function () {
            try { this.__urnArcadePacManGame?.destroy?.(); } catch (err) {
                console.warn("[URN Arcade PacMan] cleanup error", err);
            }
            this.__urnArcadePacManGame = null;
