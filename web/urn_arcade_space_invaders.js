...
const EXTENSION_NAME = "urn.arcade.space_invaders";
const NODE_NAME = "URNArcadeSpaceInvaders";
...
function createGame(node) {
    ...
    canvas.tabIndex = 0;
    canvas.setAttribute("aria-label", "URN Arcade Space Invaders. Left/right arrows to move, Space to fire.");
    ...
    function draw() {
        ...
        if(state==="idle"||state==="ready"||state==="paused"||state==="gameover"){
            ...
            let title="READY"; if(state==="idle")title="URN ARCADE INVADERS"; else if(state==="paused")title="PAUSED"; else if(state==="gameover")title="GAME OVER";
...
function attachGameSafely(node,attempt=0){
    if(!node||node.__urnArcadeSpaceInvadersGame||node.__urnArcadeSpaceInvadersAttachFailed)return;
    if(typeof node.addDOMWidget!=="function"){
        if(attempt<40)setTimeout(()=>attachGameSafely(node,attempt+1),50);
        else{node.__urnArcadeSpaceInvadersAttachFailed=true;console.error("[URN Arcade Space Invaders] addDOMWidget unavailable after 2 seconds.");}
        return;
    }
    try{
        ...
        node.__urnArcadeSpaceInvadersGame=game;
        ...
        console.info("[URN Arcade Space Invaders] game attached",node.id);
    }catch(err){node.__urnArcadeSpaceInvadersAttachFailed=true;console.error("[URN Arcade Space Invaders] game UI attach failed; node kept alive:",err);}
}
...
app.registerExtension({
    ...
    async beforeRegisterNodeDef(nodeType,nodeData){
        ...
        if(nodeType.prototype.__urnArcadeSpaceInvadersPatched)return;
        nodeType.prototype.__urnArcadeSpaceInvadersPatched=true;
        ...
        nodeType.prototype.onRemoved=function(){ try{this.__urnArcadeSpaceInvadersGame?.destroy?.();}catch(err){console.warn("[URN Arcade Space Invaders] cleanup error",err);} this.__urnArcadeSpaceInvadersGame=null; return originalRemoved?.apply(this,arguments); };
    }
});
